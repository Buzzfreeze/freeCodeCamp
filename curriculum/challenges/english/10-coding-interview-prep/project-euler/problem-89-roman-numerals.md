...
สำหรับตัวเลขที่เขียนด้วยตัวเลขโรมันที่ถือว่าถูกต้อง มีกฎพื้นฐานที่ต้องปฏิบัติตาม แม้ว่ากฎจะอนุญาตให้แสดงตัวเลขได้มากกว่าหนึ่งวิธี แต่ก็จะมีวิธีที่ *ดีที่สุด* ในการเขียนอยู่เสมอ

- ตัวเลขต้องเรียงจากมากไปน้อย
- M, C และ X ต้องไม่เท่ากับหรือน้อยกว่าตัวที่เล็กกว่า
...
- I สามารถอยู่ข้างหน้า V และ X ได้เท่านั้น
- X วางได้ก่อน L และ C เท่านั้น
- C วางได้ก่อน D และ M เท่านั้น

เช่น ดูเหมือนว่ามีอย่างน้อยหกวิธีในการเขียนเลขสิบหก:
...
แต่ ตามกฎแล้ว มีเพียง XIIIIII และ XVI เท่านั้นที่ใช้ได้ และตัวอย่างสุดท้ายถือว่ามีประสิทธิภาพมากที่สุด เนื่องจากใช้ตัวเลขน้อยที่สุด

array `roman` จะมีตัวเลขที่เขียนด้วยตัวเลขโรมันที่ถูกต้องแต่ไม่จำเป็นต้องใช้ตัวเลขน้อยที่สุด

ให้หาว่าในการเปลี่ยนการเขียนเลขโรมันที่ยาวให้สั้นลง เราจะประหยัดไปกี่ตัวอักษร

**Note:** คุณสามารถสมมติได้ว่าเลขโรมันทั้งหมดในอาร์เรย์มีหน่วยที่เหมือนกันไม่เกินสี่หน่วยติดต่อกัน

# --hints--

`romanNumerals(testNumerals1)` ต้องคืนค่าเป็นตัวเลข

```js
assert(typeof romanNumerals(_testNumerals1) === 'number');
```

`romanNumerals(testNumerals1)` ต้องคืนค่าเป็น `19`

```js
assert.strictEqual(romanNumerals(_testNumerals1), 19);
```

`romanNumerals(testNumerals2)` ต้องคืนค่าเป็น `743`
