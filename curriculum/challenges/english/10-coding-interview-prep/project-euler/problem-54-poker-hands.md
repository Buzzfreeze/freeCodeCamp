...
Poker มีไพ่ตามลำดับจากมากสุดไปน้อยสุด เป็น
...
<ul>
 ...
 ไพ่มีค่าตามลำดับนี้ 2, 3, 4, 5, 6, 7, 8, 9, 10, Jack, Queen, King, Ace.

ถ้าผู้เล่นสองคน 2 มีชุดไพ่ที่เหมือนกัน ดังนั้นชุดไพ่ที่มีคะแนนมากกว่าจะชนะ เช่น คู่แปดชนะคู่ห้า (ตัวอย่าง 1)

แต่ถ้าผู้เล่นมีชุดไพ่แบบเดียวกัน เช่น มีคู่ Q ทั้งคู่จะดูเฉพาะไพ่ที่สูงที่สุดในมือ (ตัวอย่าง 4) ถ้าถ้าไพ่ที่สูงสุดเท่ากัน ก็จะดูลำดับต่อไปเรื่อยๆ
...
ฟังก์ชันจะรับ global array (`handsArr`) ที่มีรูปแบบของไพ่ในมือแบบสุ่ม 1,000 แบบ สำหรับผู้เล่นสองคน โดยแต่ละ element จะมีไพ่ 10 ใน (แยกโดยใช้เว้นวรรค): ห้าใบแรกเป็นของผูเล่นคนที่ 1 และห้าใบหลังเป็นของผูเล่นคนที่ 2

ให้อ่านข้อมูลจาก global array นี้ แล้วบอกว่าผู้เล่น 1 ชนะกี่ครั้ง

# --hints--

`pokerHands(testArr)` ต้องคืนค่าเป็นตัวเลข

```js
assert(typeof pokerHands(testArr) === 'number');
```

`pokerHands(testArr)` ต้องคืนค่าเป็น 2

```js
assert.strictEqual(pokerHands(testArr), 2);
```

`pokerHands(handsArr)` ต้องคืนค่าเป็น 376
