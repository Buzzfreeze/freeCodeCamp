...
Triangle, square, pentagonal, hexagonal, heptagonal, and octagonal จำนวนทั้งหมดคือจำนวนมุม (polygonal) และหาค่าได้จาก:
...
1.ชุดนี้เป็นแบบวงกลมโดยที่ตัวเลขสองหลักสุดท้ายของแต่ละตัวเลขคือตัวเลขสองหลักแรกของตัวเลขถัดไป (รวมตัวเลขสุดท้ายกับตัวแรก)
...
หาผลรวมของตัวเลขทั้งหมดในชุดเรียงลำดับของ `n` cyclic 4-digit numbers for which each of theสำหรับแต่ละ $P_3$ to $P_{n + 2}$ polygonal types ถูกแทนด้วยตัวเลขอื่นในชุด

# --hints--

`cyclicalFigurateNums(3)` ควร return number.

```js
assert(typeof cyclicalFigurateNums(3) === 'number');
```

`cyclicalFigurateNums(3)` ควร return `19291`.

```js
assert.strictEqual(cyclicalFigurateNums(3), 19291);
```

`cyclicalFigurateNums(4)` ควร return `28684`.

```js
assert.strictEqual(cyclicalFigurateNums(4), 28684);
```

`cyclicalFigurateNums(5)` ควร return `76255`.

```js
assert.strictEqual(cyclicalFigurateNums(5), 76255);
```

`cyclicalFigurateNums(6)` ควร return `28684`.
