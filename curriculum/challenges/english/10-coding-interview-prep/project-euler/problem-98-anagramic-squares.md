...
โดยการแทนที่ตัวอักษรแต่ละตัวในคำว่า CARE ด้วย 1, 2, 9 และ 6 ตามลำดับ เราจะสร้างตัวเลขกำลังสอง: $1296 = 36^2$ สิ่งที่น่าทึ่งก็คือ แอนนาแกรม RACE จะใช้การแทนที่ดิจิทัลเดียวกันเพื่อสร้างตัวเลขกำลังสอง: $9216 = 96^2$ เราจะเรียก CARE (และ RACE) เป็นคู่คำแอนนาแกรมสี่เหลี่ยมและระบุเพิ่มเติมว่าไม่อนุญาตให้ใช้เลขศูนย์นำหน้า และตัวอักษรอื่นอาจมีค่าดิจิทัลเหมือนกันกับตัวอักษรอื่น

ใช้`words` array ค้นหาคู่คำsquare anagramสี่เหลี่ยมทั้งหมด (คำpalindromic  ไม่ถือเป็นsquare anagramของตัวเอง)

จำนวนกำลังสองที่ใหญ่ที่สุดที่เกิดจากสมาชิกของคู่ดังกล่าวคืออะไร?

**Note:** ทุก anagrams ที่เกิดขึ้นจะต้องอยู่ในที่กำหนด `words` array

# --hints--

`anagramicSquares(['CARE', 'RACE'])` ควร return a number.

```js
assert(typeof anagramicSquares(['CARE', 'RACE']) === 'number');
```

`anagramicSquares(['CARE', 'RACE'])` ควร return `9216`.

```js
assert.strictEqual(anagramicSquares(['CARE', 'RACE']), 9216);
```

`anagramicSquares(testWords1)` ควร return `4761`.

```js
assert.strictEqual(anagramicSquares(_testWords1), 4761);
```

`anagramicSquares(testWords2)` ควร return `18769`.
