...
ถ้าเราแทนที่ตัวอักษรแต่ละตัวในคำว่า CARE ด้วย 1, 2, 9 และ 6 ตามลำดับ เราจะได้ตัวเลขกำลังสอง: $1296 = 36^2$

สิ่งที่น่าทึ่งก็คือ แอนนาแกรม RACE (Anagram คือการเรียงตัวอักษรกลับด้าน เช่น CARE - RACE) ถ้าเราทำการแทนที่ตัวอักษรด้วยตัวเลข เราจะได้ค่าเลขกำลังสองเป็น $9216 = 96^2$

เราจะเรียก CARE (และ RACE) เป็นคู่คำแอนนาแกรมกำลังสอง และระบุเพิ่มเติมว่าไม่อนุญาตให้ใช้เลขศูนย์นำหน้า และตัวอักษรแต่ละต้องไม่มีค่าตัวเลขซ้ำกับตัวอักษรอื่น

ให้หาคู่คำแอนนาแกรมกำลังสองใน array `words`

ให้หาว่า จาก array ที่ให้ไป ค่ากำลังสองที่มากที่สุดที่ได้จากวิธีด้ายนคือเท่าใด?

# --hints--

`anagramicSquares(['CARE', 'RACE'])` ต้องคืนค่าเป็นตัวเลข

```js
assert(typeof anagramicSquares(["CARE", "RACE"]) === "number");
```

`anagramicSquares(['CARE', 'RACE'])` ต้องคืนค่าเป็น `9216`

```js
assert.strictEqual(anagramicSquares(["CARE", "RACE"]), 9216);
```

`anagramicSquares(testWords1)` ต้องคืนค่าเป็น `4761`

```js
assert.strictEqual(anagramicSquares(_testWords1), 4761);
```

`anagramicSquares(testWords2)` ต้องคืนค่าเป็น `18769`
